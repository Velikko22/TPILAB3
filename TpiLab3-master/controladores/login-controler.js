...
async function usuarioExiste() {
        ...
        .then(respuesta => {
            respuesta.forEach(usuario => {
                ...
                if (usuario.correo === inputEmail.value && usuario.password === inputPassword.value) {
                    ...
                    usuarioRole = usuario.role;/*agrega el rol*/
                    if (usuario.role === "admin"){
                        return existeUsuario = true;
                } else {
                    mostrarMensaje('Acceso denegado: no cuenta con permisos de admin')
                    return existeUsuario = true;
                }
            } else{
                return;
    ...
    if (!existeUsuario) {
        mostrarMensaje('Email o contraseña incorrectos, intente nuevamente');
    } else {
